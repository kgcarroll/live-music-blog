...
export async function runFullEditorialAlgoliaReindex(): Promise<{count: number}> {
  ...
  const appId = process.env.NEXT_PUBLIC_ALGOLIA_APPLICATION_ID?.trim()
  ...
  if (!appId) throw new Error('NEXT_PUBLIC_ALGOLIA_APPLICATION_ID is not set')
  if (!indexName) throw new Error('NEXT_PUBLIC_ALGOLIA_INDEX_NAME is not set')
  if (!adminKey) throw new Error('ALGOLIA_ADMIN_API_KEY is not set')
