...
export function usePhotoGalleryLightbox(items: PhotoGalleryImage[]) {
        ...
        <div className="relative flex w-full max-w-6xl flex-col items-center gap-3">
            ...
            {(() => {
              ...
              return (
                <div className="flex h-[80vh] w-full items-center justify-center px-10 sm:px-16">
                  <div
                    className={`inline-block max-h-[80vh] max-w-[min(95vw,calc(100vw-8rem))] overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900 transition duration-300 ease-out motion-reduce:transition-none ${
          ...
          <div className="flex max-w-2xl flex-col items-center gap-1 px-2 text-center">
            {items[openIndex]?.caption?.trim() ? (
              <p className="text-sm leading-relaxed text-amber-300">{items[openIndex].caption!.trim()}</p>
            ) : null}
            {items.length > 1 ? (
  ...
  return {
    ...
    setOpenIndex,
    close,
    lightboxPortal: lightbox ? createPortal(lightbox, document.body) : null,
  }
}
