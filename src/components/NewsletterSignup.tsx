...
const CONSENT_LINE_1 = 'Updates from Philadelphia Music Live.'
const CONSENT_LINE_2 = 'Unsubscribe anytime from any email we send.'
...
export function NewsletterSignup({
  ...
  const formBlock = (
    <form onSubmit={onSubmit} className={isFooter || isPopup ? 'mt-4' : 'mx-auto mt-4 max-w-md'}>
        <input
          type="text"
          name="website"
          tabIndex={-1}
        ...
        <p className={`mt-2 text-xs text-zinc-500 ${isFooter || isPopup ? '' : 'text-center'}`}>
          <span className="block">{CONSENT_LINE_1}</span>
          <span className="block">{CONSENT_LINE_2}</span>
        </p>
        {message ? (
          <p
            role={status === 'error' ? 'alert' : 'status'}
            className={`mt-3 text-sm ${isFooter || isPopup ? '' : 'text-center'} ${status === 'error' ? 'text-red-400' : 'text-amber-200'}`}
          >
            {message}
          </p>
        ) : null}
